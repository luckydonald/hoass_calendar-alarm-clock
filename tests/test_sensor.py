"""Tests for Calendar based Alarm Clock sensor platform."""
import pytest
...
async def test_async_setup_entry(hass, config_entry):
    ...
    assert isinstance(sensors[0], CalendarBasedAlarmClockSensor)


def test_sensor_initialization(config_entry):
    """Test sensor initialization."""
    sensor = CalendarBasedAlarmClockSensor(config_entry, "example")

    assert sensor._entry == config_entry
    assert sensor._sensor_type == "example"
    assert sensor._attr_name == "Calendar based Alarm Clock Example"
...
def test_sensor_device_info(config_entry):
    ...
    assert device_info["name"] == "Calendar based Alarm Clock"
    assert device_info["manufacturer"] == "Custom"
    assert device_info["model"] == "Calendar based Alarm Clock"
...
def test_sensor_multiple_types(config_entry):
    ...
    sensor1 = CalendarBasedAlarmClockSensor(config_entry, "type1")
    sensor2 = CalendarBasedAlarmClockSensor(config_entry, "type2")

    assert sensor1._attr_name == "Calendar based Alarm Clock Type1"
    assert sensor2._attr_name == "Calendar based Alarm Clock Type2"
