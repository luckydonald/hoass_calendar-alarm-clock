...
interface CalendarBasedAlarmClockCardConfig extends CardConfig {
  type?: string;
...
interface AppData {
  ...
  config: CalendarBasedAlarmClockCardConfig;
}

class CalendarBasedAlarmClockCardElement extends HTMLElement {
  private _config: CalendarBasedAlarmClockCardConfig = {};
  ...
  public setConfig(config: CalendarBasedAlarmClockCardConfig): void {
    this._config = config;
    if (this._app?._instance?.proxy) {
      const proxy = this._app._instance.proxy as ComponentPublicInstance & AppData;
      proxy.config = config;
  ...
  public connectedCallback(): void {
    ...
    this._app = createApp({
      ...
      render() {
        ...
        return h(CalendarBasedAlarmClockCard, {
          hass: data.hass,
          config: data.config,
        });
      },
  ...
  public static getConfigElement(): CalendarBasedAlarmClockCardEditor {
    return document.createElement('calendar-alarm-clock-card-editor') as CalendarBasedAlarmClockCardEditor;
  }

  public static getStubConfig(): CalendarBasedAlarmClockCardConfig {
    return {
      type: 'custom:calendar-alarm-clock-card',
      title: 'Calendar based Alarm Clock',
    };
  }
}

class CalendarBasedAlarmClockCardEditor extends HTMLElement {
  private _config: CalendarBasedAlarmClockCardConfig = {};
  ...
  private _renderManual(): void {
    ...
    wrapper.appendChild(this._createTextInput(
      'title',
      'Card Title',
      this._config.title ?? 'Calendar based Alarm Clock',
    ));
  ...
  private _updateConfig(update: Partial<CalendarBasedAlarmClockCardConfig>): void {
    this._config = {
      ...this._config,
      ...update,
    };
...
customElements.define('calendar-alarm-clock-card', CalendarBasedAlarmClockCardElement);
customElements.define('calendar-alarm-clock-card-editor', CalendarBasedAlarmClockCardEditor);
