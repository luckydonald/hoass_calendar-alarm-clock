...
import CalendarBasedAlarmClockCard from '../src/CalendarBasedAlarmClockCard.vue';

import type { CardConfig, HassEntity, HomeAssistant } from '../src/types';

describe('CalendarBasedAlarmClockCard', () => {
  ...
  it('renders with title', () => {
    const wrapper = mount(CalendarBasedAlarmClockCard, {
      props: {
        hass: mockHass,
        config: mockConfig,
      },
  ...
  it('uses default title when not configured', () => {
    ...
    expect(wrapper.text()).toContain('Calendar based Alarm Clock');
  });
