...
✅ **ADDRESSED**: Created troubleshooting guide in `ai/debug-logging-not-working.md`

**Quick fixes to try**:

1. **No restart needed** - Use Developer Tools:
   - Developer Tools → Services → `logger.set_level`
   - Service data: `custom_components.calendar_alarm_clock: debug`

2. **Permanent** - configuration.yaml (requires HA restart):
   ```yaml
   logger:
     default: info
     logs:
...
3. **UI method** (HA 2023.4+):
   - Settings → System → Logs → Settings → Add logger

**Common issues**: YAML indentation, need full restart, not just reload

**Verification**: Should see `INFO: Home Assistant started, checking for auto-discovery opportunity...`
